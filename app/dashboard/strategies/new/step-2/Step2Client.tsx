...
export default function StrategyStep2Client({
  rightSlot,
}: {
  rightSlot?: ReactNode;
}) {
        ...
        <div className="space-y-3">
              ...
              {trackTemplates.map((keyword) => {
                ...
                return (
                  <Badge
                    ...
                    className="cursor-pointer"
                    onClick={() =>
                      isSelected ? removeKeyword(keyword) : addKeyword(keyword)
                    }
                  >
                    {keyword}
                  </Badge>
          ...
          {showWarning && (
            <div className="rounded-2xl border border-amber-500/40 bg-amber-500/10 px-4 py-3 text-xs text-amber-600">
              Add at least one subreddit or keyword to help the crawler focus.
            </div>
          )}
        </div>
      </div>
