...
export async function GET(request: Request) {
  ...
  const { data, error } = await supabase
    .from('trends')
    .select('keyword, title, tags, status, score, status_reason')
    .eq('slug', slug)
    .eq('is_public', true)
    .maybeSingle();
  ...
  const title = (data.title as string | null) ?? (data.keyword as string);
  ...
  return new ImageResponse(
    (
      ...
      >
        ...
        {data.status_reason && (
          <div style={{ marginTop: 20, fontSize: 24, color: '#475569', maxWidth: '1000px' }}>
            {data.status_reason}
          </div>
        )}
